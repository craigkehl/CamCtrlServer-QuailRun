...
class PTZCameraModel {
  ...
  readonly CMD_BASE: number[] = [0x81, 0x01];
  readonly CMD_END: number = 0xff;
  ...
  //#region Power
  powerOn(): number[] {
    const command: number[] = [...this.CMD_BASE];
  ...
  zoom(rate: number): number[] {
    const command: number[] = [...this.CMD_BASE]; //0x81 0x01
  ...
  presetGet(n: number): number[] {
    const command: number[] = [...this.CMD_BASE, 0x04, 0x3f, 0x02];
    command.push(n);
    command.push(this.CMD_END);
    return command;
  }
  ...
  presetSet(n: number): number[] {
    const command: number[] = [...this.CMD_BASE, 0x04, 0x3f, 0x01];
    command.push(n);
    command.push(this.CMD_END);
    return command;
  }
  ...
  presetSpeed(n: number): number[] {
    const command: number[] = [...this.CMD_BASE, 0x06, 0x20];
    command.push(n);
    command.push(this.CMD_END);
    return command;
  }
  ...
  moveVarSpeed(xRate: number, yRate: number): number[] {
    const command: number[] = [...this.CMD_BASE, 0x06, 0x01];
  ...
  //#region Get Status
  readonly QUERY_BASE: number[] = [0x81, 0x09];
